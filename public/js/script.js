  ...
  feedback_star_rating.forEach((star) => {
    star.onmouseover = () => {
      ...
      if (star.className.indexOf(starClassUnactive) !== -1) {
        ...
        for (star_rating_index; star_rating_index >= 0; --star_rating_index)
          feedback_star_rating[star_rating_index].className = starClassActive;
      } else {
        printRatingResult(feedBackRatting, star_rating_index);
        for (
          star_rating_index;
          star_rating_index < starsLength;
          ++star_rating_index
        )
          feedback_star_rating[star_rating_index].className = starClassUnactive;
      }
    };
  });
}
